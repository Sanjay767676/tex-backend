...
// Enhanced email template with plain text version support
...
const sendConfirmationEmail = async ({
    ...
    const replyToEmail = senderType === 'NCS' ? process.env.NCS_EMAIL_USER : process.env.CS_EMAIL_USER;
    ...
    // Log PDF buffer info
    console.log(`[Email Service] PDF Buffer - Size: ${pdfBuffer?.length || 0} bytes, Valid: ${pdfBuffer && pdfBuffer.length > 0}`);
...
const sendLunchEmail = async ({
    ...
    // Log PDF buffer info
    console.log(`[Email Service] Lunch PDF Buffer - Size: ${pdfBuffer?.length || 0} bytes, Valid: ${pdfBuffer && pdfBuffer.length > 0}`);
    ...
    try {
        console.log(`[Email Service] 🍽️ Sending lunch token to: ${to}`);
        console.log(`[Email Service] 📋 Using sender: ${fromName} <${fromEmail}>`);
